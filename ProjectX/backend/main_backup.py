...
async def process_user_question(question: str, session_id: Optional[str] = None):
    """Process a user question using the agentic SQL analyst logic"""

    # This is a simplified version - in production, you'd use proper LLM tool calling
    # For now, we'll implement a basic rule-based approach to demonstrate the concept

    logger.info(f"Processing question: {question}")

    # Simple intent detection (in production, use LLM with function calling)
    question_lower = question.lower()

    if "revenue" in question_lower and ("month" in question_lower or "time" in question_lower):
        # Handle revenue by month query
        sql = """
        SELECT
        ...
        # Execute the query
        result = await run_sql(SQLQueryRequest(query=sql, read_only=True))

        # Generate response
        answer = f"Here's the revenue by month for the last 6 months:"
        ...
        if result["rows"]:
            ...
            # Render chart
            chart_result = await render_chart(chart_spec)

            # Pin insight automatically
            await pin_insight(
                title="Monthly Revenue Trend",
                summary=f"Shows revenue trend over the last {len(result['rows'])} months",
                chart_or_table_ref=chart_result["chart_spec"],
                sql=sql
            )
            ...
            return {
                ...
                "sql": sql,
                "notes": "Data shows monthly revenue calculated from order items quantity multiplied by unit price. Limited to last 6 months.",
                "next_steps": [
                    "Want to see this broken down by product category?",
                    "Should I check if this trend holds when excluding refunds?"
                ]
            }
        else:
            return {
                "answer": "I couldn't retrieve revenue data. The query returned no results.",
                "detail": {},
                "sql": sql,
                "notes": "Query executed but returned no data. This could be due to date filters or missing data.",
                "next_steps": [
                    "Want me to check the date range of available order data?",
                    "Should I look at total revenue without date restrictions?"
                ]
            }

    elif "best customers" in question_lower:
        # Handle best customers query with assumption statement
        answer = "I'll assume 'best customers' means customers with highest lifetime revenue."
        ...
        if result["rows"]:
            chart_spec = ChartSpec(
                chart_type="bar",
                data=result["rows"][:5],  # Top 5 for chart
                x="customer_name",
                y="lifetime_revenue",
                title="Top 5 Customers by Lifetime Revenue"
            )
            ...
            await pin_insight(
                ...
                summary="Shows customers ranked by their total lifetime value",
                chart_or_table_ref=chart_result["chart_spec"],
                sql=sql
            )
            ...
            return {
                ...
                "sql": sql,
                "notes": "Lifetime revenue calculated as sum of quantity * unit_price from all orders. Limited to top 10 customers.",
                "next_steps": [
                    "Want to see this broken down by region or segment?",
                    "Should I check purchase frequency vs total spend?"
                ]
            }
        else:
                return {
                    "answer": answer + " However, no customer data was found.",
                    "detail": {},
                    "sql": sql,
                    "notes": "Query executed but returned no customer/order data.",
                    "next_steps": [
                        "Want me to check if customer and order tables have data?",
                        "Should I look at raw counts in these tables?"
                    ]
                }
        ...
        return {
            "answer": f"I received your question: '{question}'. I'm currently set up to handle specific types of queries like revenue analysis and customer ranking. For more complex questions, I'd need to use the full LLM-powered agent with proper tool calling.",
            "detail": {},
            "sql": "-- No SQL generated for this question type",
            "notes": "This is a demo implementation. A production version would use Anthropic Claude with function calling to dynamically generate and execute SQL based on the question.",
            "next_steps": [
                "Try asking about revenue by month",
                "Try asking about best customers",
                "Or provide more specific details about what you'd like to analyze"
            ]
        }
